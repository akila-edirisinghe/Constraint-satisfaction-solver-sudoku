...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        smallest_domain = grid.get_width()
        list_small = []
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) < smallest_domain and len(grid.get_cells()[i][j]) != 1:
                    smallest_domain = len(grid.get_cells()[i][j])
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) == smallest_domain:
                    list_small.append((i,j))
        
        return random.choice(list_small)
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while len(Q) != 0:
            ...
            row = cell[0]
            column = cell[1]
            variables_assigned_row, failure_row = self.remove_domain_row(grid, row, column)
            variables_assigned_column, failure_column = self.remove_domain_column(grid, row, column)
            variables_assigned_unit, failure_unit = self.remove_domain_unit(grid, row, column)
            if failure_row or failure_column or failure_unit:
                return True
            for i in variables_assigned_row:
                Q.add(i)
            for i in variables_assigned_column:
                Q.add(i)
            for i in variables_assigned_unit:
                Q.add(i)
        return False
    ...
    def search(self, grid, var_selector):
        ...
        for d in grid.get_cells()[var[0]][var[1]]:
            if grid.is_value_consistent( d, var[0], var[1]):
                ...
                Q = set()
                Q.add(var)
                if not ac3.consistency(copy_A, Q):
...
for p in problems:
    ...
    h = backtracking.search(g, var_selector)
    '''
    if h != False:
        h.print()
    if h.is_solved():
    ...
    '''
    first_end = time.time()
    first_done = first_end - first_start
    first_time.append(first_done)
...
for p in problems:
    ...
    '''
    mrv_end = time.time()
    mrv_done = mrv_end - mrv_start
    mrv_time.append(mrv_done)


plotter = PlotResults()
plotter.plot_results(mrv_time, first_time,
